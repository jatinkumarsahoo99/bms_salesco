...
class MarkROsFlagController extends GetxController {
  ...
  void saveTodayAndAllData(bool fromSaveToday) {
    if (selectedLocation == null || selectedChannel == null) {
      ...
      Get.find<ConnectorControl>().POSTMETHOD(
        ...
        fun: (resp) {
          closeDialogIfOpen();
          if (resp != null && resp is Map<String, dynamic> && resp['save']['isError'] != null && !(resp['save']['isError'] as bool)) {
        ...
        json: {
          ...
          "saveTodayAllDays": dataTableList.value.map((e) => e.toJson(fromSave: true)).toList(),
        },
      );
    }
  }
  ...
  handleOnChangedLocation(DropDownValue? val) {
    ...
    if (val != null) {
      ...
      Get.find<ConnectorControl>().GETMETHODCALL(
        ...
        fun: (resp) {
          closeDialogIfOpen();
          if (resp != null && resp['channel'] != null && resp['channel'] is List<dynamic>) {
            channelList.clear();
            selectedChannel = null;
  ...
  getOnLoadData() {
    Get.find<ConnectorControl>().GETMETHODCALL(
        ...
        fun: (resp) {
          closeDialogIfOpen();
          if (resp != null && resp['location'] != null && resp['location'] is List<dynamic>) {
  ...
  void handleGenerateButton() {
    if (selectedLocation == null || selectedChannel == null) {
      ...
      LoadingDialog.call();
      Get.find<ConnectorControl>().GETMETHODCALL(
        api: ApiFactory.MARK_ROS_FLAG_GET_DATA(selectedLocation?.key ?? "", selectedChannel?.key ?? "",
            DateFormat("yyyy-MM-ddT00:00:00").format(DateFormat("dd-MM-yyyy").parse(effectiveDateTC.text))),
        fun: (resp) {
          closeDialogIfOpen();
          madeChanges = false;
          if (resp != null && resp['show'] != null && resp['show'] is List<dynamic>) {
            dataTableList.clear();
            dataTableList.value.addAll((resp['show'] as List<dynamic>).map((e) => MarkROSFlagModel.fromJson(e)).toList());
          } else {
            LoadingDialog.showErrorDialog(resp.toString());
          }
        },
