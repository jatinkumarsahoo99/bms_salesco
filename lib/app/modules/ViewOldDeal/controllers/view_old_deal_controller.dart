...
class ViewOldDealController extends GetxController {
        ...
        fun: (map) {
          ...
          if (map is Map &&
            ...
            if (map['pageload'].containsKey('lstlocations') &&
                map['pageload']['lstlocations'] != null &&
                map['pageload']['lstlocations'].length > 0) {
            ...
            if (map['pageload'].containsKey('lstpersonnelmaster') &&
              ...
              map['pageload']['lstpersonnelmaster'].forEach((e) {
                executiveList.add(DropDownValue.fromJsonDynamic(
                    e, "personnelcode", "executivename"));
              });
  ...
  ViewOldDealResponseModel? viewOldDealResponseModel =
      ViewOldDealResponseModel(deal: Deal(lstdealmaster: [], lstdealusage: []));
          ...
          if (map is Map && map.containsKey('deal') && map['deal'] != null) {
            ...
            if (viewOldDealResponseModel?.deal?.lstdealmaster != null &&
                (viewOldDealResponseModel?.deal?.lstdealmaster?.length ?? 0) >
                    0) {
              if ((viewOldDealResponseModel?.deal?.lstdealmaster?[0].fromDate !=
                      null &&
                  viewOldDealResponseModel?.deal?.lstdealmaster?[0].fromDate !=
                      "")) {
                dealPeriodFromController.text = convertDateFromat(
                    viewOldDealResponseModel
                            ?.deal?.lstdealmaster?[0].fromDate ??
                        "");
              }
              if ((viewOldDealResponseModel?.deal?.lstdealmaster?[0].todate !=
                      null &&
                  viewOldDealResponseModel?.deal?.lstdealmaster?[0].todate !=
                      "")) {
                toDateFromController.text = convertDateFromat(
                    viewOldDealResponseModel?.deal?.lstdealmaster?[0].todate ??
                        "");
              }
              dealAmountController.text = (viewOldDealResponseModel
                          ?.deal?.lstdealmaster?[0].dealamount ??
                      "0")
                  .toString();
              totalDealDurationController.text =
                  (viewOldDealResponseModel?.deal?.lstdealmaster?[0].seconds ??
                          "0")
                      .toString();
            }
            update(['grid']);
