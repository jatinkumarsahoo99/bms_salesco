...
class DataGridFromMapForAmagiSpotReplacement extends StatelessWidget {
  DataGridFromMapForAmagiSpotReplacement(
      {Key? key,
        ...
        this.isMasterGrid = false,this.onContextMenuClick})
      : super(key: key);
  final FocusNode? previousWidgetFN;
  final Map<String, double>? widthSpecificColumn;
  final Map<String, double>? specificWidth;
  ...
  Widget build(BuildContext context) {
    ...
    for (var key in mapData[0].keys) {
      segColumn.add(PlutoColumn(
          ...
          enableColumnDrag: false,
          field: key,
          type: (numTypeKeyList != null && numTypeKeyList!.isNotEmpty &&
              numTypeKeyList!.contains(key))? PlutoColumnType.number() : PlutoColumnType.text()  ));
    }
...
class DataGridFromMapAmagiDialog extends StatelessWidget {
  DataGridFromMapAmagiDialog(
      {Key? key,
        ...
        this.specificWidth,
        this.numTypeKeyList,
        this.widthSpecificColumn,this.summary = false})
      : super(key: key);
  final FocusNode? previousWidgetFN;
  final Map<String, double>? widthSpecificColumn;
  final Map<String, double>? specificWidth;
...
Color getColors(PlutoColumnRendererContext plutoCon){
  ...
  if(plutoCon.cell.column.title.toString().trim().toLowerCase() != "Parent I D".toLowerCase()){
    List<String>? data = plutoCon.stateManager.rows[plutoCon.rowIdx].cells['ParentID']?.value.toString().split("-");
    print(">>>>>>>>>splitList$data");
    if(data != null && data.isEmpty){
      color = Colors.white;
    }
    else if(plutoCon.cell.value == null || plutoCon.cell.value.toString().trim() == "null" ||
        plutoCon.cell.value.toString().trim() == ""){
      color = Colors.pink;
    }else if(double.parse( (plutoCon.cell.value??"0").toString()) < double.parse( data![1]??"0")){
...
FontWeight getChannelFont(PlutoColumnRendererContext plutoCon){
 ...
 if(plutoCon.stateManager.rows[plutoCon.rowIdx].cells[("${(plutoCon.column.field??"").toString().trim()}IsBold")]?.value.toString().trim() == "true"){
